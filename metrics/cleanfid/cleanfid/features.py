...
import cleanfid.cleanfid
from cleanfid.cleanfid.downloads_helper import *
from cleanfid.cleanfid.inception_pytorch import InceptionV3
...
def feature_extractor(name="torchscript_inception", device=torch.device("cuda"), resize_inside=False):
    if name == "torchscript_inception":
        ...
        model = InceptionV3W("C:\PhD\SR\CYCLEGAN_PIX2PIX_TORCH\pytorch-CycleGAN-and-pix2pix\metrics\cleanfid",download=True, resize_inside=resize_inside).to(device)
        model.eval()
